...
1. **Truth Store**
   ...
   - **Store interface**: ContextStore exposes queryNodes, getProposal, createProposal, submitReview, applyProposal, traverseReasoningChain, detectConflicts, mergeProposals, isProposalStale. See [AGENT_API.md](AGENT_API.md), [../reference/DATA_MODEL_REFERENCE.md](../reference/DATA_MODEL_REFERENCE.md).

2. **Policy & Governance**
   - **RBAC**: workspace roles (Reader, Contributor, Reviewer, Applier, Admin); agents get propose only, never review/apply.
...
Concurrent proposals → detectConflicts(proposalId) → mergeable vs needsResolution; optional mergeProposals(ids); isProposalStale(proposalId) for optimistic locking. See [../appendix/RECONCILIATION_STRATEGIES.md](../appendix/RECONCILIATION_STRATEGIES.md).
...
- **File-backed**: Git-friendly; JSON/YAML; atomic writes and locking per workspace.
- **Database-backed**: MongoDB recommended for v1; indexes on workspaceId + nodeId, revisionId, proposal status; apply as transaction.
