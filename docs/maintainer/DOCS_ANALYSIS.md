...
From **docs/README.md**, **docs/ARCHITECTURE.md**, **docs/WHITEPAPER.md**:
...
- **One minimal governance UI is required** (list proposals, accept/reject/apply). **Agent Runtime is optional** (query truth, author proposals; cannot accept/reject/apply).
...
|------|------------------|
| **README.md** | Leads with "Enterprise truth governance"; "truth ledger + collaboration layer"; ACAL; minimal governance UI required; Agent Runtime optional; points to docs/ as canonical. |
| **CONTEXT.md** | context-001 and goal-001 use docs/ framing (truth ledger + ACAL, minimal governance UI required, Agent Runtime optional). |
| **PLAN.md** | Intro states docs/ is source of truth; Phase 5 = "Contextualize + Agent Runtime (optional)"; broken section refs (§4, §7.4, §7.5, DOCX §5–7) removed or replaced with file-only refs. |
...
- A dedicated "Agent loop" subsection in CONTEXTUALIZED_AI_MODEL if the optional agent runtime is expanded.
...
- **docs/README, ARCHITECTURE, WHITEPAPER**: Truth ledger + collaboration, ACAL, minimal governance UI required, Agent Runtime optional.
...
- **Terminology** — Use docs/ terms in new root/PLAN text (truth ledger, ACAL, minimal governance UI, Agent Runtime optional).
...
| docs/README | Doc index + ACAL | Canonical. |
| docs/ARCHITECTURE | Components (Truth Store, Policy, Projection, Change Detection, Clients, Agent Runtime optional) | Canonical. |
| docs/WHITEPAPER | Problem, solution, ACAL, enterprise | Canonical. |
| docs/CONTEXTUALIZED_AI_MODEL | Context for agents; leakage policy | Canonical. |
| docs/AGENT_API | Safe agent contract | Canonical. |
| docs/UI_SPEC | Minimal governance UI | Canonical. |

---

**Conclusion:** The **docs/ folder is the source of truth**. Root README, CONTEXT, and PLAN have been aligned to docs/ (enterprise truth governance, truth ledger + collaboration layer, ACAL, minimal governance UI required, Agent Runtime optional). Broken section references in PLAN have been removed or replaced with file-level links. No further changes to docs/ are required for consistency.
