...
- **Projections**: Markdown, DOCX, or HTML views generated from truth—readable, versionable, shareable. Edits to projections can become proposals via change detection. See [appendix/CHANGE_DETECTION.md](appendix/CHANGE_DETECTION.md)- **The Agent (we build it)**: an agent that reads accepted truth and creates/updates proposals; it cannot submit reviews or apply. One minimal governance UI is required for humans to govern. **The solution provides an MCP server** so AI assistants (Cursor, Claude Desktop, etc.) can use TruthLayer as a native tool—query truth, create proposals, traverse reasoning—from the IDE and other MCP clients. See [core/AGENT_API.md](core/AGENT_API.md), [core/UI_SPEC.md](core/UI_SPEC.md).
