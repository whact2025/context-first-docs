...
- **Implementation status.** Data subject rights workflows (export by subject, rectification via proposal, erasure/redaction procedures) are **designed** and reflected in data model and guardrails (see [Security & Governance](SECURITY_GOVERNANCE.md) § Personal data sensitivity, Immutability with redaction). Operational playbooks and tooling (e.g. “erase by data subject id”) are part of the implementation roadmap where not yet delivered.
...
- **Implementation status.** Retention **model** (classes, configurable policies) is designed and referenced in [Security & Governance](SECURITY_GOVERNANCE.md) § Data handling, Retention awareness. Full retention **engine** (scheduled jobs, per-workspace policy UI) is on the implementation roadmap.
...
| **Audit log export**      | Immutable audit logs; exportable per workspace and date range in a standard format.                                | Designed; export format and automation on roadmap.                                                    |
