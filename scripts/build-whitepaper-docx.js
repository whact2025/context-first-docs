 ...
 *
 * Usage: node scripts/build-whitepaper-docx.js [--skip-pandoc] [--scale N]
...
/** Document list: whitepaper first, then appendix, then supporting docs. */
const DOC_LIST = [
 ...
 */
async function processMermaidInContent(content, prefix, outputDir, scale) {
  const matches = [...content.matchAll(MERMAID_BLOCK_RE)];
  if (matches.length === 0) return { content, imageCount: 0 };
...
async function main() {
  const { skipPandoc, scale } = parseArgs();
  ...
  for (const { file, prefix } of DOC_LIST) {
    ...
    const { content: afterMermaid, imageCount } = await processMermaidInContent(content, prefix, outputDir, scale);
